/* v_task38_r1_scan.c -- exact, COMPLETE scan closing residue R1 of
...
static void test_all(long long n,i128 C3,long long l){
    ...
    for(int f1=1;f1<=2;f1++){
        ...
        if((q=sh_L3(I))&&q>=3){ nshape++; shf[0]++;
            if((i128)(q+1)>g*f1){ ngapwin++; finish("L3","L3",n,q,l,f1,3,"R=(q+1)/f1"); } }
        /* L4 / S4 share iota1 = (q+1)(q^2+1) */
        if((q=sh_q3(I))&&q>=2){ nshape++; shf[1]++;
            if((i128)q*q+q+1 > g*(i128)(q+1)*f1){ ngapwin++; finish("L4","L4",n,q,l,f1,6,"R=(q^2+q+1)/((q+1)f1)"); }
            f=prime_power(q,&p);
            if(f){
                if(p!=2&&f==1&&q>=5&&f1==1){ if((i128)q*q > g*2*(i128)(q+1)){ ngapwin++; finish("S4p","S4",n,q,l,1,4,"R=q^2/(2(q+1))"); } }
                else if(p!=2&&f>=2&&f1==1){ i128 t2=2*(i128)(q-1)/(i128)(p-1); if(t2>g){ ngapwin++; finish("S4np","S4",n,q,l,1,4,"R<=2t(q)"); } }
                else if(p==2&&f>=2){ i128 t2=(i128)q-1; if(t2>g){ ngapwin++; finish("S4e","S4",n,q,l,f1,4,"R=t(q)<=q-1"); } }
            } }
        /* L5 */
        if((q=sh_cyc(I,5))&&q>=2){ nshape++; shf[2]++;
            if((i128)q*q+1>g){ ngapwin++; finish("L5","L5",n,q,l,f1,10,"R=q^2+1"); } }
        if(f1>1) continue;
        /* U4 */
        if((q=sh_U4(I))&&q>=3){ nshape++; shf[3]++;
            if((i128)q*q+1 > g*(i128)(q+1)){ ngapwin++; finish("U4","U4",n,q,l,1,6,"R=(q^2+1)/(q+1)"); } }
        /* Sp6 */
        if((q=sh_cyc(I,6))&&q>=2){ nshape++; shf[4]++; f=prime_power(q,&p);
            if(f==1){ if((i128)q*q*q*q-1 > g*((i128)q*q*q-1)){ ngapwin++; finish("Sp6p","Sp6",n,q,l,1,9,"R=(q^4-1)/(q^3-1)"); } }
            else if(f>=2){ i128 t2=2*(i128)(q-1)/(i128)(p-1); if(t2>g){ ngapwin++; finish("Sp6np","Sp6",n,q,l,1,9,"R<=2t(q)"); } } }
        /* U5 */
        if((q=sh_U5(I))&&q>=2){ nshape++; shf[5]++; f=prime_power(q,&p);
            if(f==1){ if((i128)q*q*q*q > g*(i128)(q+1)*((i128)q*q+1)){ ngapwin++; finish("U5p","U5",n,q,l,1,10,"R=q^4/((q+1)(q^2+1))"); } }
            else if(f>=2){ i128 t2=(i128)(q-1)/(i128)(p-1); if(t2>g){ ngapwin++; finish("U5np","U5",n,q,l,1,10,"R=t(q)"); } } }
    }
}
/* ---------------- divisors <= bound ---------------- */
...
int main(int argc,char**argv){
    ...
    printf("# v_task38 R1 scan -- complete over ALL q, for 10 <= n <= %lld\n",NMAX);
