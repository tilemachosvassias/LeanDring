# Statement alignment of `simpleGroupTheorem`'s five inputs

Does the paper-level mathematics actually instantiate the Lean `Prop`s of
`LeanDring/Even/SimpleRigidity.lean`?  This is a read-only comparison of the
two statements, Prop by Prop; no Lean file is changed by it.

Section numbers of the form §4xx below refer to sections of the private
development record, where the corresponding mathematics is written up;
"Müller" is Müller, Diss. Jena 2008.

**Summary.**  Four of the five Props are aligned as stated.  The two
...
ring theory".  **That claim is false as written.**
...
`LeanDring/Even/SimpleRigidity.lean`
...
The proof body is five lines and is kernel-checked; the question below is
entirely whether the five hypotheses are the statements the mathematics
proves.

---

## 1. `SimpleOrderRigidity Exc` — aligned, with two mandatory
##    side-conditions on how `Exc` is instantiated

### (a) Exact Lean statement (`SimpleRigidity.lean`)
...
* **Tits convention: harmless.**  Whether `²F₄(2)′` is counted as sporadic
  or Lie-type changes nothing: `sporadic_twins.py` verified that the Tits group's
...
## 2. `ExceptionSeparation Exc` — a shape mismatch, and a hidden
...
1. **`{B_n(q), C_n(q)}`, `n ≥ 3`, `q` odd — Theorem BC-n** (§4bl +
   `bc_separation` / `bc_separation_recheck`):
...
* **⚠ MISMATCH 1 — shape.**  Every Lean deliverable in the BC lane
  concludes `IsEmpty (DRing B ≃+* DRing C)` (`BCSeparation.lean`,
  `BoltjeAssembly.lean`), never `∃ c, conductorMultiplicity … ≠ …`.
  ...
  It would follow from one short lemma
...
* **⚠ GAP 2 — the dictionary is required, and only half-documented.**  The
  ...
  `isIndexValue_iff_of_boltje` (`ConductorAtSpecies.lean`), which is
  **`BoltjeIntegrality`-conditional**.  This is correctly documented in
  `BoltjeAssembly.lean` and in the development record — but *not* in
  `SimpleRigidity.lean`, whose `ExceptionSeparation` and `simpleGroupTheorem`
  docstrings present the Prop as "a per-pair census … external to the ring theory".  It is not
...
* **⚠ NOTE 3 — do not cite the §4j(iii) A₈/L₃(4) argument.**  §4j(iii)
  ...
  proven-tier separation is the `ι` one above.
...
* **Coverage: complete.**  With `Exc` restricted as in §1, the two members
  ...
  paper level (Theorem BC-n, re-derived in `bc_separation_recheck`).  ✓

---

## 3. `OrderTransfer` — aligned

### (a) Exact Lean statement (`Even/Separation.lean`)
...
| link | file | status |
|---|---|---|
| `mullerOrder_eq_of_ringEquiv` | `OrderTransfer.lean` | proven, no hypotheses |
| `conductorMultiplicity_eq_of_ringEquiv` | `OrderTransfer.lean` | proven, no hypotheses |
| `MullerTrivialBound` | `MullerBound.lean` | proven (Burnside triangularity) |
| `MullerIsolationWeak` (⊃ needed half) | `ConductorDictionaryProofs.lean` | open |
| `orderTransfer_of_muller'` | `ConductorDictionaryProofs.lean` | proven from the two above |
...
implies the other; two short wiring lemmas
(`mullerIsolationWeak_of_boltje`, `orderTransfer_of_boltje`) would close it.

---

## 4. `FittingFreeDetector` — aligned; assumed, never proven, and also
##    reducible to the dictionary
...
(the identifier occurs only in `SimpleRigidity.lean`); it is a pure
cited-literature hypothesis — assumed, not proven.
...
## 5. `TwinSeparation` — aligned in content, but with a dictionary
##    dependency, a shape mismatch, and scope caveats
...
|---|---|---|
| `A_n`, all `n ≥ 9` | Theorem AT (§4aj, §4ba + addenda 1–2) | `ind*` value set / `κ`; (H2′) discharged |
| 26 sporadics + Tits | Theorem SPT (§4bb, `sporadic_twins.py`) | 14 rows census-empty; 10 by orbit floor; 3 by floor + `c ∣ ∏|Out|` |
...
* **⚠ GAP 1 — the dictionary dependency (taken up in §6 below).**  Every non-vacuous row is separated
...
* **⚠ MISMATCH 2 — shape (`¬ D-iso` vs `∃ c`).**  The paper theorems are
  stated as *non-existence*: Theorem SPT — "*There is no
  ...
  - *orbit-floor rows*: the floor is derived from `P ⊆ I(T)` (Lemma TR-1,
    an unconditional identity, `sporadic_twins.py`) plus arithmetic, giving
...
* **Scope caveats inherited from the paper** (must survive into the public
  ...
  on exact sweeps at named bounds; `Lemma C` is proven (`lemma_c_bound.py`)
  and no longer a debt; `(AB)`/`(AB-S)` are discharged for `A_n` but
...
conductor campaign.  The mathematics states the dependency ("*By Satz 2.3.3 the pair `(H, hH')` has conductor
`(N_G(H,hH') : H')` … For `G` Fitting-free the conductor `|G|` occurs
exactly once (Satz 2.3.6) so `κ(G)` is precisely the second-largest
conductor*"), and `sporadic_twins.py` repeats it as `(†)`.
...
*Where it is documented.*  `Even/BoltjeAssembly.lean` and the development
record document it **for the BC-n and Theorem-AT-endgame consumers** — that is the whole point of the
...
opposite ("*Five typed inputs, all external to the ring theory*"; "*Both inputs are
external to the ring theory*"; "*None of the three is a gap in D-ring
theory*").  **This is the single most important correction recorded here.**
...
> `ConductorAtSpecies.lean`, consumed by `BoltjeAssembly.lean`).  The
...
| Prop | alignment | one-line reason |
|---|---|---|
...
| `ExceptionSeparation Exc` | shape mismatch + dictionary dependency | BC-n yields `IsEmpty (DRing B ≃+* DRing C)`, not `∃ c`; the `ind*` → `conductorMultiplicity` step is `BoltjeIntegrality`-conditional |
...
| `TwinSeparation` | content aligned; dictionary dependency + shape mismatch + full-CFSG citation | census-empty rows are vacuous and dictionary-free; every other row needs Satz 2.3.3/2.3.6 and a witness-extraction lemma that does not exist |
