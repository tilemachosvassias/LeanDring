...
# -*- coding: utf-8 -*-
r"""
v_task88_pos.py -- closing the (POS) residue of Theorem MF in the LINEAR
tower: subfield factors of type ^2A (= unitary groups over a proper subfield
of F_q), which are the 753 784 rows v_task87_pos.py could not exclude with
the orbit floor alone.

v_task87 section 4.1 closed the odd-m half of the residue by hand and left
...
residue, by three exact ingredients.
  ...
  PART 1  Exact cyclotomic data: Cyc(U_m) and its multiplicities, k(U_m),
          ...
          v_task60's cyc_of_tokens / target_cyc.
  ...
  PART 5  The (POS)-free version: with Lemma FL2 (v_task88_h6.py PART 2)
          ...
          v_task88_h6.py PART 4.  Numerical exhaustion: all (m, j, f) shapes
          for 61 <= d <= DMAX, plus exact integer spot rows 61 <= d <= 120.

Usage:  python3 v_task88_pos.py [dmax] [dspot]      (default 2000 120)
"""
...
from v_task56_lu_twins import KFAM, Kmax, iota_LU, order_LU
from v_task60_lu_residues import cyc_of_tokens, TERMS, target_cyc
...
def part3(dlo=61, dhi=2000):
    ...
    print(BAR)
    print(r"""
  Lemma PACK (v_task60 section 1.3), over the common base u = p^g with
...
def part4(survivors, dlo=61, dhi=2000):
  ...
  REMAINING factors (which satisfy (POS)):
     ...
     m = d   :  sum_{i>=2} v_i       = (d-1) - (d-1)/2 = (d-1)/2
                ...
                [this is v_task87 section 4.1's odd-m half]
...
def part6(nonowner):
  ...
  PARTS 2-4 do not apply to it.  By Lemma FL2 (v_task88_h6.py PART 2) the
...
def main():
    ...
    print("v_task88_pos.py -- the ^2A half of the (POS) residue")
