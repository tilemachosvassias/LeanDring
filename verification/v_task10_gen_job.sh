#!/bin/bash
# usage: gen_job.sh <groupname> <atlasname> <idx> <outbase>
GROUPNAME=$1
ATLASNAME=$2
IDX=$3
OUTBASE=$4
cat > ${OUTBASE}.g << GEOF
Print("=== Job: ${GROUPNAME} maximal #${IDX} ===\n");
LoadPackage("atlasrep");
G := AtlasGroup("${ATLASNAME}");
M := AtlasSubgroup("${ATLASNAME}", ${IDX});
if M = fail then
...
Print("=== Job done: ${GROUPNAME} maximal #${IDX} ===\n");
GEOF
