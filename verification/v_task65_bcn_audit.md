# v_task65 — ADVERSARIAL AUDIT of Theorem BC-n (v_task64, §4bl)

Date 2026-08-08.  Scripts `v_task65_bcn_audit.g` (GAP 4.16, CTblLib),
`v_task65_bcn_audit.py`; outputs `v_task65_bcn_audit_out.txt`,
`v_task65_bcn_audit_py_out.txt`.  **NOTES.md not edited** (per brief).
Everything below was re-derived independently; the v_task64 scripts were
read only *after* my own runs, for comparison.

## VERDICT: Theorem BC-n SURVIVES.

`D(B_n(q)) ≇ D(C_n(q))` for all `n ≥ 3`, all odd `q`.  One genuine
**ERRATUM** in the write-up (item 3: the exclusion is stated over `I(S)`
but must be stated over the whole conductor multiset), repairable in one
paragraph with no change to the conclusion; two presentational tightenings
(items 1b, 2a).  No break of the v_task63 kind — I looked for the analogue
of the "omitted column" and there is none: unlike the `mult₁` route, the
exclusion here is a *dichotomy over all subgroups*, not an inventory.

---

## 1. The B-side witness — CONFIRMED
...
**(b) Spinor norm — CONFIRMED, with a normalisation erratum.**
...
I rebuilt `r_e` from the reflection formula
...
So the brief's "the ε and class flip with q mod 4" is really *the class of
...
**(c) `N_S(⟨z_e⟩) = C_S(z_e) = Stab_S(⟨e⟩)` — CONFIRMED, and this is a
gap I had to close myself.**  `|⟨z_e⟩| = 2` so `N = C` automatically.  For
...
brief's doubling worry does not occur.

Note `v_task64`'s script never computed a centraliser: it printed
`Stabilizer(G, point)` and *asserted* it equals `C_S(z_e)`.  My script
...
maxes list for `O7(3)` — the v_task64 remark that "O7(3) has no stored
...
`ind* = [S:N_S(H)]` (v_task37 def. line 41). ✓

**Extra: n = 4 checked** (not done in v_task64, which stopped at n = 3).
`Ω₉(3)`: the realised orbit is `3321 = v_{−1}`, `ι₁ = 3280 ∤ 3321`,
`q^{2n} = 6561`. The witness is not an n = 3 accident. ✓

## 2. The C-side exclusion — CONFIRMED (with a tightening)
...
(v_task64 read this off AtlasRep SLPs and got `364, 1120` — consistent,
but its list was truncated; CTblLib's is complete and confirms.)

*General n, class by class* (`v_task65_bcn_audit.py` step (B)): I rebuilt
the index bounds from Kleidman–Liebeck Table 3.5.C orders — **including
the two classes v_task64's table omits, `C₄` (`Sp_{2a} ⊗ O_b`) and `C₇`
...
artifacts of my own slack, both disposed of exactly**:
...
* `S` class at `n = 3`: v_task64's "`|M| ≤ 4f·max(...)` so index `≫ q⁶`
...
The tightest *exact* case is as v_task64 says: `[Sp₆:P₃] = (q+1)(q²+1)
...
**(b) `ι₁ ∤ v_ε` — CONFIRMED, and by the right (arithmetic) argument.**
The brief is right that a pure size argument is insufficient
(`v_ε/ι₁ ≈ q/2` is an integer-sized candidate).  The correct proof, which
**v_task64 does give** in its §4: `2v_ε = q^n(q^n−ε)` and `gcd(ι₁,q) = 1`,
...
**(c) `PSp` vs `Sp` — CONFIRMED.**  Indices are unchanged by the central
...
`H = S`, `ind* = |S|`. ✓  Note the `|H'| > 1` weighting the brief worries
about is harmless precisely because divisibility is preserved by
multiplication — the brief's own analysis is right.
...
could in principle be carried by a non-identity coset.  v_task64 asserts
...
## 4. Scope and the q = 3 anchor — CONFIRMED
...
* `351 ∈ I(Ω₇(3))` — my own orbit/centraliser computation, and `2.U₄(3).2₂`
...
## 5. Break hunt — nothing found

Places I probed for the v_task63-style "omitted column":
...
* Non-abelian `H` with `|H'| > 1` engineered to hit `v_ε` — covered by
  divisibility (§2 dichotomy), as the brief anticipated.
* Non-identity cosets — the one real erratum, §3, repaired.
* `C₄`/`C₇` (absent from v_task64's table) — indices verified far above
  `q^{2n}`.
...
| item | verdict |
|---|---|
| 1a `det z_e = 1` | **CONFIRMED** |
| 1b spinor norm / square class | **CONFIRMED**; ERRATUM on `disc` normalisation (factor `2^{2n+1}`), data consistent |
| 1c `N = C = Stab`, orbit size, no index-2 drop | **CONFIRMED** (computed directly; v_task64 had asserted it) |
| 1d `ind*` weight for abelian `H` | **CONFIRMED** |
| 2a only-`P₁` below `q^{2n}` | **CONFIRMED**; tightening required for `C₆` and for the `n=3` `S` class (v_task64's single max-order bound is too coarse at `q = 3`); `C₄`,`C₇` omitted from its table |
| 2b `ι₁ ∤ v_ε` | **CONFIRMED** (arithmetic form, correct in v_task64) |
| 2c `PSp` bookkeeping | **CONFIRMED** |
| 3 D-invariance of the exclusion | **ERRATUM** — must range over all conductor pairs, not `I(S)`; repaired above |
| 4 scope + `q=3` anchor | **CONFIRMED** |
| 5 break hunt | no break found |
| **Theorem BC-n** | **CONFIRMED**, modulo the §3 restatement and the cited classification of maximal subgroups of `Sp_{2n}(q)` |
