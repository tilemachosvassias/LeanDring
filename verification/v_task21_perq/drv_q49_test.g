...
DerivedIndexRecog := function(M)
    local gens, comms, i, j, Hgens, mgens, ri, changed, newgens, h, g, img,
          rounds, zelt, zmem, sz, t0, elapsed, roundcap, reason;
    t0 := Runtime();;
    ...
    while changed do
        ...
        ri := RecogniseGroup(Group(Hgens));
        if ri = fail then reason := "RECOG_FAIL"; break; fi;
        newgens := [];; changed := false;;
        for g in mgens do
            for h in Hgens do
                img := h^g;
                if SLPforElement(ri, img) = fail then
    ...
    sz := Size(ri);;
    zelt := -One(M);;
    zmem := (SLPforElement(ri, zelt) <> fail);;
