# v_task6: Audit of (A) the n=3 closed-form r-formula and (B) the cyclic-faithful intrinsicness claim

Date: 2026-08-05. GAP 4.16.0, python3. Falsification-first audit; all scripts/logs
in this directory (`v_task6_*`).

## Task A — VERDICT: MATCH (both groups)

Claim under audit: the closed-form formula in the exploratory `task1_check.g`
predicts, for the extraspecial groups of order 128 = 2^{1+6},
r(plus) = 27632 and r(minus) = 27136. These values had never been checked
against actual groups.

Method (non-circular): generic engine from `v_task4_rationality.g` —
...
formula's stratification. Script: `v_task6_extraspecial128.g`, log:
`v_task6_extraspecial128.log.txt`.
...
| group | type | subgroup classes | computed r | prediction | verdict |
|---|---|---:|---:|---:|---|
| SmallGroup(128,2326) | plus  | 2996 | **27632** | 27632 | **MATCH** |
| SmallGroup(128,2327) | minus | 2898 | **27136** | 27136 | **MATCH** |
...
Claim under audit: "the set of cyclic-faithful species
...
Method: exact exhaustive Aut(D(G)) enumeration via `python/fullaut3.py`
(rerun from source; deps `latauto/swaprigid/dring2` all present). Script:
`v_task6_cfcheck.py`, log: `v_task6_cfcheck_out.txt`; deep reruns of the two
capped groups: `v_task6_cfcheck_deep.py` / `v_task6_cfcheck_deep_out.txt`.

Parametrization/translation (stated per task instructions): fullaut3's
...
The Lean claim under audit is about the IDEAL, not the species set:
`brauerKernel := { x : s(x) = 0 for all cyclic-faithful species s }`, with
the trivial pair INCLUDED per the auditee's own definition (= primary
...
`v_task6_idealcheck.py`, log: `v_task6_idealcheck_out.txt`.
...
(direct check on the kernel lattice). So for the auditee's own definition
...
* The MOVED/PRESERVED dichotomy for Q8 flips with the convention on the
  trivial pair; the claim as transmitted did not fix the convention, so both
