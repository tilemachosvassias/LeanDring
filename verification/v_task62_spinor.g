#############################################################################
##  v_task62_spinor.g   --  degree-7 spinor-norm square classes
##
##  For each of the five *rational* degree-7 orthogonal columns
##      U in { A_8, U_3(3), L_3(2), L_2(8), 2^6:A_7 }
##  decide, per odd q, how many Omega_7(q)-classes of perfect
##  self-normalizing subgroups isomorphic to U the column contributes.
##
##  Theory (see v_task62_spinor.md, section 1):
##    * U perfect  ==>  U <= Omega_7(q) automatically (det x theta is abelian).
##    * C_{SO_7}(U) = 1 (absolute irreducibility; the only orthogonal scalar
##      of determinant 1 is the identity), so N_{SO}(U)/U embeds in
##      Stab_{Out(U)}([module]).
##    * # Omega-classes per Out-orbit of modules:
##         Stab = 1                       -> 2 classes, both psn
##         Stab <> 1, realised in Omega   -> 0 classes (not psn)
##         Stab <> 1, realised outside Om -> 1 class, psn
##      "realised in Omega" = the isometry T inducing the outer automorphism,
##      normalised to det = +1, has SQUARE spinor norm.
##
##  Spinor norm is computed by the Wall form,  theta(g) = (-2)^k det(chi_g),
##  k = rank(g-1), chi_g(x,y) = B(x',y) with x = (g-1)x'.  Validated below
##  against reflections, homomorphy, and Omega(0,7,q) membership.
#############################################################################

LogTo("v_task62_spinor_out.txt");
...
Print("== 1. validation of the spinor-norm routine ==\n");
for q in [3,5,7,9,11,13,17,19,23,25,27] do ValidateSpinor(q); od;
...
Print("\n== 2. the five rational columns ==\n");

## ANCHOR COLUMNS (must reproduce the sec.4ai addendum: 2 classes each,
## every odd q -- the "doubled Sp_6(2) tree")  and  L_2(13) (the A3 column).
anchors := [ rec(name:="2^3:L3(2)",  G := Image(IsomorphismPermGroup(PerfectGroup(1344,1)))),
             rec(name:="2^3.L3(2)",  G := Image(IsomorphismPermGroup(PerfectGroup(1344,2)))),
             rec(name:="2^6:L3(2)",  G := Image(IsomorphismPermGroup(PerfectGroup(10752,8)))),
             rec(name:="L2(13)",     G := PSL(2,13)) ];

groups := [ rec(name:="A8",     G := AlternatingGroup(8)),
            rec(name:="L3(2)",  G := PSL(3,2)),
            rec(name:="U3(3)",  G := PSU(3,3)),
            rec(name:="L2(8)",  G := PSL(2,8)),
            rec(name:="2^6:A7", G := Image(IsomorphismPermGroup(
                                    PerfectGroup(161280,1)))) ];

QS := [3,5,7,9,11,13,17,19,23,25,27,29,31,37,41,43];

results := [];
for r in Concatenation(anchors, groups) do
  Print("\n--- ", r.name, "  |G| = ", Size(r.G), " ---\n");
  r.aut := AutomorphismGroup(r.G);
  r.inn := InnerAutomorphismsAutomorphismGroup(r.aut);
  r.out := Filtered(List(RightTransversal(r.aut, r.inn)), a -> not a in r.inn);
  Print("  |Out| = ", Length(r.out)+1, "\n");
  for q in QS do
    Add(results, ColumnData(r.G, r.aut, r.out, q, r.name));
  od;
od;

Print("\n== 3. summary table ==\n");
for r in results do
  Print(r.name, "\t q=", r.q, "\t classes=", r.classes, "\n");
od;
