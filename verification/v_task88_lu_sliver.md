# v_task88 — the last named sliver of the LU tower: **(H6)** and **(POS)**

Date 2026-08-09.  **`NOTES.md` not edited.**
Input: `v_task87_d33_residual.md` (Theorem MF, Lemma COX, gaps (H6) and (POS)),
`v_task82_audit_block2.md` §2.4, `v_task60_lu_residues.md` (Lemma DEG, Lemma
ZM, Lemma PACK / PACK-TOP, DEG conventions), `v_task56_lu_twins.md` §8.

| file | what it does |
|---|---|
| `v_task88_h6.py` / `_out.txt` | **(H6)**: PART 1 Lemma OUT-p (`p ∤ d_i`, so `v_p(c)` loses its rank-sized term); PART 2 **Lemma FL2** (the orbit floor bounds the *number* of socle factors: `L ≤ 7` linear, `L ≤ 3` unitary, `L ≤ 2` / `L ≤ 1` with no exceptional factor); PART 3 the exact slack calculus `(S1)`–`(S3)`, machine-verified as identities of rationals; PART 4 the tolerance of every branch of Theorem MF and the **integer kill** of the one tight branch; PART 4b how much the floor alone empties; PART 5 the exact slack-free census extended to `61 ≤ d ≤ 120`, every `q ≤ 100` |
| `v_task88_pos.py` / `_out.txt` | **(POS)**: PART 1 exact cyclotomic data; PART 2 the owner equation + positivity kill `j > 1/4`; PART 3 **(PACK-exc) in base-field indices**, which kills every `j ≠ 1/2`; **PART 3c the same three tests over ALL 17 families** (only `²A` survives); PART 4 the COX-mean kill of `j = 1/2`, **both parities of `m`** (the even-`m` half is the sharper substitute for `h ≤ 2v` that `v_task87` §7.2 asked for); PART 5 exact integer spot rows `61 ≤ d ≤ 120`; PART 6 the non-owner subfield rows, completed exactly |
...
> PART 4.  The even-`m` half — the piece `v_task87` left open — is closed by
> the exact identity
>
> > `∑_{i≥2} v_i (d − 1 − h_i) = 1`,
>
...
> `q ≤ 100` — **0 surviving configurations**, on top of `v_task87`'s
> `33 ≤ d ≤ 60`.
>
...
`v_task60` PART C — and Theorem MF after it — use Lemma DEG in the form
...
Verified exhaustively (`v_task88_h6.py` PART 1) on **46 155** rows (all 17
...
`ρ = n_r/μ` per family (computed, `v_task88_h6.py` PART 2):
...
`v_task88_h6.py` **PART 4c**, which subsumes all three linear branches and
...
`v_task88_h6.py` PART 4b: over `61 ≤ d ≤ 120` and every prime power
...
`v_task88_h6.py` PART 5 runs `v_task87` §5's four integer tests
...
Together with `v_task87` §5 (`33 ≤ d ≤ 60`) this makes the branch
**unconditionally empty for `33 ≤ d ≤ 120` and every `q ≤ 100`**.

---

## 2. (POS): the residue `v_task87` §7.2 named

`v_task87_pos.py` found `753 784` `(family, rank, f_i < f)` rows that the
orbit floor alone cannot exclude in the linear tower, dominated by `²A`
(unitary factors over a proper subfield).  `v_task87` §4.1 closed the odd-`m`
half of the `Φ_K`-owner case by hand and left the even-`m` half open,
"needs a sharper substitute for `h_{i₀} ≤ 2 v_{i₀}`".
...
Both verified against `v_task60`'s `cyc_of_tokens` / `target_cyc`
(`v_task88_pos.py` PART 1, `3 ≤ m ≤ 200`, `4 ≤ d ≤ 200`: 0 violations).
...
threshold in `q`.  Machine run (`v_task88_pos.py` PART 3), `61 ≤ d ≤ 800`,
...
**`m = d` (odd, `v_task87` §4.1's half).**  `∑_{i≥2} v_i = (d−1)/2` and
...
`v_task88_pos.py` **PART 3c** runs the three tests over every
...
parametrised list, which `v_task88_pos.py` PART 6 completes and tests with
...
`v_task88_pos.py` PART 5, `61 ≤ d ≤ 120`, `p ∈ {2,3,5,7,11,13}`,
...
| **Theorem MF unconditional** | `31 ≤ d ≤ 120`, every `q ≤ 100` | **[PROVEN]** (census, §1.8 + `v_task87` §5) |
...
2. **(H6): the (L-c) kill assumes (POS)** for the two factors of the pinned
   shape, and uses PACK-TOP's uniqueness (`M_K = 1`, verified `d ≤ 300` in
   `v_task87`) and the observation that `C_{d/2}` does not cover `Φ_{d−1}`.
...
3. **(H6) for `d ≤ 60`** is not re-derived here; it is covered by the exact
   censuses (`v_task87` §5 for `33 ≤ d ≤ 60`, `v_task82` for `21 ≤ d ≤ 32`,
   `v_task56`/`v_task60` below) — all of which are `q`-bounded
   (`q ≤ 100`, resp. `q ≤ 3000`, `q ≤ 10⁵`).

4. **`d ≥ 121`, and the sweep's own grid.**  §1.6's decisive sweep was run
...
6. **(H1), (H5) unchanged** (`v_task56` §8): `ι`'s lower half is literature
...
```
python3 v_task88_h6.py  120 100   > v_task88_h6_out.txt      # ~30 min
python3 v_task88_pos.py 2000 120  > v_task88_pos_out.txt     # ~5 min
```

`v_task88_h6.py` imports `FAM`/`order_of` from `v_task52_lemmaC`,
`KFAM`/`Kmax`/`iota_LU`/`order_LU` from `v_task56_lu_twins`;
`v_task88_pos.py` additionally imports `cyc_of_tokens`/`TERMS`/`target_cyc`
from `v_task60_lu_residues` — i.e. both are checked against the same family
and cyclotomic tables the audited scripts use.
