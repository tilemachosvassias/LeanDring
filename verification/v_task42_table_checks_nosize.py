#!/usr/bin/env python3
"""v_task42 (derived from v_task38_table_checks.py): the SAME branch checks
(a)/(b1)/(b2)/(b3) with the SIZE filter |S| > C(n,4) REMOVED, since that
filter came from Theorem TO Step 1, which used the side condition (*).
Theorem TO' uses no (*), so SIZE is not available.  Consequences:
  * checks() no longer applies "order > C(n,4)";
  * in the n-free branches (b2)/(b3) the bound on n can no longer come from
    Lemma TO-1 (which used SIZE); it comes instead from the ORDER cap
    ell = C(n,4)/(f1*i1) <= log(n!/2)/log|S|, which is monotone in n, so the
    loop runs until the cap first fails.
...
def checks(name,q,p,f,i1,order,N,f1,n,target):
    ...
    l = target//(f1*i1)
    if l<1: return False
    # v_task42: SIZE filter (order > C(n,4)) removed -- it used (*)
        ...
        for (q,p,f) in PP:
            ...
            for (f1,f2) in pairs:
                ...
                for tag,val in (("b2",F(3)),("b3",F(4)),("b3",F(4,3))):
                    ...
                    n=10
                    while True:
                        # ORDER cap: ell = C(n,4)/(f1*i1) grows ~ n^4, the cap
                        # log(n!/2)/log|S| ~ n log n, so the test is monotone.
                        if C(n,4)/(f1*i1) * log(order) > lgamma(n+1)-log(2):
                            break
                        if C(n,4)%(f1*i1)==0:
                            r=checks(name,q,p,f,i1,order,N,f1,n,C(n,4))
                            if r: rows[key][tag].append((q,n,r[0],str(val)))
                        n+=1
                        if n>10**6: print("  !! n-loop cap hit",name,q,tag); break
