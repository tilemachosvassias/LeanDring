...
/-!
# Discharging `CharsAtComplete` by decided exhaustiveness (`k = 3`, chunk g)

Kernel `decide` of `exhaustiveCheck` over representatives `103…110` (each with `k = 3`
...
/-- **Exhaustiveness for representatives `103…110`.** -/
theorem exhaustiveCheck_r103_110 : (List.range' 103 8).all exhaustiveCheck = true := by decide
