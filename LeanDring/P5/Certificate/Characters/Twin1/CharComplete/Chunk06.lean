...
/-!
# Discharging `CharsAtComplete` by decided exhaustiveness (`k = 3`, chunk f)

Kernel `decide` of `exhaustiveCheck` over representatives `95…102` (each with `k = 3`
...
/-- **Exhaustiveness for representatives `95…102`.** -/
theorem exhaustiveCheck_r95_102 : (List.range' 95 8).all exhaustiveCheck = true := by decide
