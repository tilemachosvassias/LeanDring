...
/-!
# Discharging `CharsAtComplete` by decided exhaustiveness (`k = 3`, chunk e)

Kernel `decide` of `exhaustiveCheck` over representatives `87…94` (each with `k = 3`
...
/-- **Exhaustiveness for representatives `87…94`.** -/
theorem exhaustiveCheck_r87_94 : (List.range' 87 8).all exhaustiveCheck = true := by decide
