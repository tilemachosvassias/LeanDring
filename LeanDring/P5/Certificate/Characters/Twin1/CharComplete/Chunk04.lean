...
/-!
# Discharging `CharsAtComplete` by decided exhaustiveness (`k = 3`, chunk d)

Kernel `decide` of `exhaustiveCheck` over representatives `79…86` (each with `k = 3`
...
/-- **Exhaustiveness for representatives `79…86`.** -/
theorem exhaustiveCheck_r79_86 : (List.range' 79 8).all exhaustiveCheck = true := by decide
