...
`Twin2/PairClass.lean` delivered the **surjection** `pairClass_realized_by_orbitRep`: every
`c : CharPairClass (Coordinate 2) ℂˣ` is the class of the character at some
...
* `card_charPairClass_coordinate2 : Nat.card (CharPairClass (Coordinate 2) ℂˣ) = 2724`
* `rank_dring_coordinate2 : Module.rank ℤ (DRing (Coordinate 2)) = 2724`.
...
* `ConjLabelProp` + `conjLabelProp_of_mem_N` — the **character↔vector dictionary
  ...
  (`normLabel p' = normLabel p`) whose character is `charForm2 … p` precomposed with
  conjugation by `a`. The generator step is `charForm2_genAct`; the induction is
  closed under products, and under **inverses via the finite-order identity**
  `a⁻¹ = a ^ (orderOf a - 1)` (`Coordinate 2` is finite), sidestepping any need for
...
* `orbitRep_class_inj_same` — the **same-representative** case: an `IsConjPair` on a
  ...
  identifies the two orbit-rep positions' characters, `charOfVec2_inj` + `Nodup`
...
subgroups. -/
theorem conjSub_of_isConjPair {K K' : Subgroup (Coordinate 2)}
    {l : K →* ℂˣ} {l' : K' →* ℂˣ}
    (h : IsConjPair K K' l l') : ∃ a : Coordinate 2, conjSub a K = K' := by
  obtain ⟨a, hmem, _⟩ := h
  refine ⟨a, ?_⟩
  ext y
...
def ConjLabelProp (r : Fin 148) (a : Coordinate 2) : Prop :=
  a ∈ Subgroup.normalizer ((reps r : Subgroup (Coordinate 2)) : Set (Coordinate 2)) ∧
    ∀ (p : Nat), p < (repChars2.getD r.val []).length →
      ∃ p', ∃ _ : p' < (repChars2.getD r.val []).length,
        (normLabel.getD r.val []).getD p' 0 = (normLabel.getD r.val []).getD p 0 ∧
        ∀ x : Coordinate 2, x ∈ reps r →
          charForm2 (basisAt2 r) ((repChars2.getD r.val []).getD p' []) x
            = charForm2 (basisAt2 r) ((repChars2.getD r.val []).getD p [])
                ((a⁻¹ * x * a : Coordinate 2))
...
theorem actPos_lt (r : Fin 148) (k p : Nat)
    ...
    ((normActPos.getD r.val []).getD k []).getD p 0 < (repChars2.getD r.val []).length := by
...
`charForm2_genAct` and the certified orbit action `act_fact` / `label_stable`. -/
theorem ConjLabelProp.base (r : Fin 148) (k : Nat)
  ...
  have hp'lt : p' < (repChars2.getD r.val []).length := actPos_lt r k p hk_mats hp
  have genActeq : genAct r.val k ((repChars2.getD r.val []).getD p [])
      = (repChars2.getD r.val []).getD p' [] := (act_fact r k p hk_mats hp).1
  have he' : charOK2 (basisAt2 r) (genAct r.val k ((repChars2.getD r.val []).getD p []))
      = true := by
    rw [genActeq]; exact validAt r (listedAt r hp'lt)
  refine ⟨p', hp'lt, label_stable r k p hk_mats hp, fun x hx => ?_⟩
  have hcf := charForm2_genAct r k hk (validAt r (listedAt r hp)) he' hx
  rw [genActeq] at hcf
  exact hcf

theorem ConjLabelProp.mul (r : Fin 148) {a b : Coordinate 2}
...
theorem ConjLabelProp.pow (r : Fin 148) {a : Coordinate 2} (ha : ConjLabelProp r a) :
...
theorem ConjLabelProp.inv (r : Fin 148) {a : Coordinate 2} (ha : ConjLabelProp r a) :
...
theorem conjLabelProp_of_mem_N (r : Fin 148) {a : Coordinate 2}
    (ha : a ∈ Subgroup.normalizer ((reps r : Subgroup (Coordinate 2)) : Set (Coordinate 2))) :
    ConjLabelProp r a := by
  have ha' : a ∈ Subgroup.closure {x : Coordinate 2 | x ∈ normGens.getD r.val []} := by
    have hof : ofList 2 hq2 (normGens.getD r.val [])
        = Subgroup.closure {x : Coordinate 2 | x ∈ normGens.getD r.val []} := rfl
  ...
  | mem x hx =>
      ...
      have hgen : normGenAt r.val k = x :=
        (congrArg (fun c : Coordinates => (c : Coordinate 2))
...
theorem normLabel_rep (r : Fin 148) (p : Nat) (hp : p < (repChars2.getD r.val []).length)
...
theorem orbitRep_class_inj_same (r : Fin 148) {q q' : Nat}
    (hq : q < (repChars2.getD r.val []).length) (hq' : q' < (repChars2.getD r.val []).length)
    (hqrep : (normIsRep.getD r.val []).getD q false = true)
    (hq'rep : (normIsRep.getD r.val []).getD q' false = true)
    (h : (⟦⟨reps r, charOfVec2 r ((repChars2.getD r.val []).getD q [])
            (validAt r (listedAt r hq))⟩⟧ : CharPairClass (Coordinate 2) ℂˣ)
        = ⟦⟨reps r, charOfVec2 r ((repChars2.getD r.val []).getD q' [])
            (validAt r (listedAt r hq'))⟩⟧) :
    q = q' := by
  set B := basisAt2 r with hB
  set vq := (repChars2.getD r.val []).getD q [] with hvq
  set vq' := (repChars2.getD r.val []).getD q' [] with hvq'
  obtain ⟨a, hmem, hchar⟩ := Quotient.exact h
  -- `a` normalizes `reps r`
  have haN : a ∈ Subgroup.normalizer ((reps r : Subgroup (Coordinate 2)) : Set (Coordinate 2)) :=
    Subgroup.mem_normalizer_iff.mpr hmem
  have hconjmem : ∀ {x : Coordinate 2}, x ∈ reps r → a⁻¹ * x * a ∈ reps r := by
    intro x hx
    exact ((Subgroup.mem_normalizer_iff''.mp haN) x).mp hx
  -- transported character equality: `charForm2 vq' (a k a⁻¹) = charForm2 vq k`
  have hval : ∀ k : Coordinate 2, (hk : k ∈ reps r) →
      charForm2 B vq' (a * k * a⁻¹) = charForm2 B vq k := by
    intro k hk
    have hc := hchar k hk
    rw [charOfVec2_apply, charOfVec2_apply] at hc
    exact zexp_inj hc
  have hstar : ∀ x : Coordinate 2, x ∈ reps r →
      charForm2 B vq' x = charForm2 B vq (a⁻¹ * x * a) := by
    intro x hx
  ...
  -- `charForm2 (repChars2 p') = charForm2 vq'` on `reps r`, so the characters agree
  have heqChar : charOfVec2 r ((repChars2.getD r.val []).getD p' []) (validAt r (listedAt r hp'))
      = charOfVec2 r vq' (validAt r (listedAt r hq')) := by
    refine MonoidHom.ext (fun z => ?_)
    obtain ⟨x, hx⟩ := z
    rw [charOfVec2_apply, charOfVec2_apply]
    congr 1
    rw [hcharP x hx, hstar x hx]
  have hvv : (repChars2.getD r.val []).getD p' [] = vq' :=
    charOfVec2_inj r (listedAt r hp') (listedAt r hq') heqChar
  -- `Nodup` collapses positions
  have hpq : p' = q' := by
    rw [hvq', List.getD_eq_getElem _ _ hp', List.getD_eq_getElem _ _ hq'] at hvv
    exact (charsAt2_nodup r).getElem_inj_iff.mp hvv
...
def orbRepSigma : Type :=
  Σ r : Fin 148, {q : Fin (repChars2.getD r.val []).length //
...
noncomputable def orbRepClass (i : orbRepSigma) : CharPairClass (Coordinate 2) ℂˣ :=
  ⟦⟨reps i.1, charOfVec2 i.1 ((repChars2.getD i.1.val []).getD i.2.1.val [])
...
theorem card_fibre_eq (r : Fin 148) :
    Nat.card {q : Fin (repChars2.getD r.val []).length //
        (normIsRep.getD r.val []).getD q.val false = true} = orbitCountR r.val := by
  have hlen : (normIsRep.getD r.val []).length = (repChars2.getD r.val []).length := by
    ...
    exact hb
  have hmap : (List.finRange (repChars2.getD r.val []).length).map
      (fun q : Fin (repChars2.getD r.val []).length => (normIsRep.getD r.val []).getD q.val false)
...
/-- **The pair count.** `Nat.card (CharPairClass (Coordinate 2) ℂˣ) = 2724`. -/
theorem card_charPairClass_coordinate2 :
    Nat.card (CharPairClass (Coordinate 2) ℂˣ) = 2724 := by
  rw [← Nat.card_eq_of_bijective orbRepClass orbRepClass_bijective, card_orbRepSigma]

/-- **The rank of `D(Coordinate 2)`.** Unconditional, via the conditional corollary
`rank_dring_coordinate2_of_card` fed the pair count. -/
theorem rank_dring_coordinate2 :
    Module.rank ℤ (DRing (Coordinate 2)) = 2724 :=
  rank_dring_coordinate2_of_card card_charPairClass_coordinate2
