...
/-!
# q=2 class-invariance checks : each conjugation row preserves the class label.

Five sift-free `getD`-only kernel `decide`s (one per generator row), plus the five
row-length facts, isolated from the sift-heavy conjugation rows and the tree/count checks.
-/
