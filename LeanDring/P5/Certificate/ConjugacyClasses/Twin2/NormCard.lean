...
/-!
# The normalizer-card bridge and Link 1 of the normalizer-orbit semantic layer

This file closes the first of the two deferred normalizer-orbit semantic links (see
`Twin1/NormOrbitData.lean`'s scope note): that the emitted normalizer index
`normIdxData r` genuinely presents `N_G(reps r)`.
...
* `card_subtype_fin_eq` / `finRange_map_classOfData` / `conjIdxCount` /
  ...
  subtype to the certified class-data count `classSizeNat`.
* `conjSub_iff_markClass` — subgroup conjugacy (`conjSub`) is exactly mark-class
  equality (`⟦⟨·,1⟩⟧`), tying the certified class action to the abstract `card_conjClass_mul`.
...
**Scope.** Link 2 (the emitted matrices `normMats` realise the conjugation action
at the vector level) is *not* closed here: the matrices were emitted from
`normBasis B`'s generators, which need not be the canonical inventory base
`getB (normIdxData r)`, so the direct kernel check fails. See `NormLink.lean`.
-/
...
/-! ## Subgroup conjugacy ↔ mark-class equality ↔ certified class label -/

/-- `∃ g, conjSub g A = B` iff the mark classes of `B` and `A` coincide. -/
theorem conjSub_iff_markClass (A B : Subgroup (Coordinate 2)) :
    (∃ g : Coordinate 2, conjSub g A = B) ↔
      ((⟦⟨B, 1⟩⟧ : CharPairClass (Coordinate 2) ℂˣ) = ⟦⟨A, 1⟩⟧) := by
  constructor
  · rintro ⟨g, rfl⟩
...
/-- Conjugacy of two inventory subgroups is detected by the certified class label. -/
theorem conj_iff_classOf (i j : Nat) (hi : i < 676) (hj : j < 676) :
    (∃ g : Coordinate 2, conjSub g (subAt i) = subAt j) ↔ classOfD i = classOfD j := by
  constructor
  · rintro ⟨g, hg⟩
...
theorem markClass_reps_iff (r : Fin 148) (i : Nat) (hi : i < 676) :
    ((⟦⟨subAt i, 1⟩⟧ : CharPairClass (Coordinate 2) ℂˣ) = ⟦⟨reps r, 1⟩⟧) ↔
      classOfD i = r.val := by
...
members — the abstract class count equals the certified class count. -/
theorem card_conjClass_reps (r : Fin 148) :
    Nat.card {L₂ : Subgroup (Coordinate 2) //
        (⟦⟨L₂, 1⟩⟧ : CharPairClass (Coordinate 2) ℂˣ) = ⟦⟨reps r, 1⟩⟧} = classSizeNat r.val := by
    ...
    (fun x : {i : Fin 676 // classOfD i.val = r.val} =>
      (⟨subAt x.1.val, (markClass_reps_iff r x.1.val x.1.isLt).mpr x.2⟩ :
        {L₂ : Subgroup (Coordinate 2) //
          (⟦⟨L₂, 1⟩⟧ : CharPairClass (Coordinate 2) ℂˣ) = ⟦⟨reps r, 1⟩⟧})) ?_).symm
  constructor
...
theorem card_coordinate2 : Nat.card (Coordinate 2) = 3125 := by
...
theorem normalizer_card_pin (r : Fin 148) :
    classSizeNat r.val *
      Nat.card (Subgroup.normalizer ((reps r : Subgroup (Coordinate 2)) : Set (Coordinate 2)))
      = 3125 := by
  have h := card_conjClass_mul (reps r)
  rw [card_conjClass_reps r, card_coordinate2] at h
  exact h

/-! ## Link 1 — the emitted normalizer index is genuinely `N_G(reps r)` -/

/-- Link-1 sift checker: every generator `n` of the candidate normalizer base
...
def normSiftCheck (r : Nat) : Bool :=
  ...
  Nb.all (fun n => B.all (fun g =>
    siftB 2 B (conjC 2 n g) && siftB 2 B (conjC 2 (invC 2 n) g)))

/-- Extract the two sift facts for a `(n, g)` pair. -/
theorem normSift_at (r : Fin 148) (hsift : normSiftCheck r.val = true)
    {n : Coordinate 2} (hn : n ∈ getB (normIdxData.getD r.val 0))
    {g : Coordinate 2} (hg : g ∈ getB (classRepsData.getD r.val 0)) :
    siftB 2 (getB (classRepsData.getD r.val 0)) (conjC 2 n g) = true ∧
      siftB 2 (getB (classRepsData.getD r.val 0)) (conjC 2 (invC 2 n) g) = true := by
...
/-- **Link 1, ≤-direction.** The candidate normalizer base sits inside the true
...
theorem Ncand_le_norm (r : Fin 148) (hsift : normSiftCheck r.val = true) :
  ...
  have hrepclo : (reps r : Subgroup (Coordinate 2))
      = Subgroup.closure {x : Coordinate 2 | x ∈ getB (classRepsData.getD r.val 0)} := by
    rw [show (reps r : Subgroup (Coordinate 2)) = subAt (classRepsData.getD r.val 0) from rfl,
      subAt_eq_closure]
  rw [SetLike.mem_coe, hrepclo]
  apply normalizes_closure hq2 n {x : Coordinate 2 | x ∈ getB (classRepsData.getD r.val 0)}
  · intro g hg
    have hs := (normSift_at r hsift hn hg).1
    have hmem := (siftB_getB_iff hreplt (conjC 2 n g)).mp hs
    rwa [conjC_eq hq2 n g] at hmem
  · intro g hg
    have hs := (normSift_at r hsift hn hg).2
    have hmem := (siftB_getB_iff hreplt (conjC 2 (invC 2 n) g)).mp hs
    rw [conjC_eq hq2 (invC 2 n) g, show invC 2 n = n⁻¹ from rfl, inv_inv] at hmem
    exact hmem

/-- **Link 1.** The emitted normalizer index `normIdxData r` presents exactly
`N_G(reps r)`. -/
...
/-- **Link 1, unconditional (relative to the sift certificate).** Given the
