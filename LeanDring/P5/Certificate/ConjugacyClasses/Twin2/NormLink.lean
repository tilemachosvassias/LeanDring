...
/-!
# Orbit partition, link 2, discharged: the orbit matrices realize genuine normalizer conjugation

This closes the second (and last) deferred normalizer-orbit semantic link, resolving the
data-provenance gap recorded in `NormCard.lean`'s header. The emitted conjugation
matrices `normMats r` were built (in `Generators/Twin2/NormGen.lean`) from the
coordinate generators of `normBasis B` — an echelon base of `N_G(reps r)` — but
those generators are not literally the canonical inventory base
`getB (normIdxData r)` that link 1 certified. Here the matrices are certified
against **their own** generating set `normGens r`.
...
* `normGenLenCheck_all` — its length equals that of the link-1 base;
* `normRealizeCheck_all` — `normMats r = (normGens r).map (conjMatrixL2 B)`.

These are cheap (small bases, `ZMod 25` micro-matrices): the four whole-range
kernel `decide`s are cheap, so they live in this one file.

## Semantic layer

* `normGens_generate` — `⟨normGens r⟩ = N_G(reps r)`. (a) membership gives
  `⟨normGens r⟩ ≤ N`; (b)+(c) give `|⟨normGens r⟩| = 5^len = |N|`; equal order +
  `≤` ⟹ equality (`Subgroup.eq_of_le_of_card_ge`), then link 1's
  `normAt_eq_final`. So `normGens r` genuinely generates the normalizer.
* `normMats_realize` — the per-entry realization
  `(normMats r)[j][i] = expVec_B (nⱼ⁻¹ · gᵢ · nⱼ)` with `nⱼ = (normGens r)[j]`,
  `gᵢ = (basisAt2 r)[i]`, `B = basisAt2 r`. Orientation: `n⁻¹ g n`.
...
/-- **(a)** Every `normGens r` element sifts into the link-1 normalizer base. -/
...
/-- **(c)** `normGens r` and the link-1 base have equal length. -/
...
theorem normGenMem_at (r : Fin 148) {g : Coordinate 2} (hg : g ∈ normGens.getD r.val []) :
    siftB 2 (getB (normIdxData.getD r.val 0)) g = true := by
...
theorem normGenWf_at (r : Fin 148) : WF 2 hq2 (normGens.getD r.val []) := by
  have h := (List.all_eq_true.mp normGenWfCheck_all) r.val (List.mem_range.mpr r.isLt)
  simp only [normGenWfCheck] at h
  exact wf_of_wfB' hq2 _ h

/-- Order match: `normGens r` and the link-1 base have equal length. -/
...
theorem normGens_generate (r : Fin 148) :
    ofList 2 hq2 (normGens.getD r.val []) =
  ...
  have hle : ofList 2 hq2 (normGens.getD r.val []) ≤ subAt (normIdxData.getD r.val 0) := by
    refine (Subgroup.closure_le _).mpr ?_
    intro g hg
    exact (siftB_getB_iff hlt g).mp (normGenMem_at r hg)
  have hcardG : Nat.card (ofList 2 hq2 (normGens.getD r.val []))
      = 5 ^ (normGens.getD r.val []).length := card_ofList hq2 _ (normGenWf_at r)
  have hcardN : Nat.card (subAt (normIdxData.getD r.val 0))
      = 5 ^ (getB (normIdxData.getD r.val 0)).length := card_subAt hlt
  have heq : ofList 2 hq2 (normGens.getD r.val []) = subAt (normIdxData.getD r.val 0) := by
...
/-- **Realization (per entry).** With `B = basisAt2 r`, `nⱼ = (normGens r)[j]`,
...
theorem normMats_realize (r : Fin 148) {j i : Nat}
    (hj : j < (normGens.getD r.val []).length) (hi : i < (basisAt2 r).length) :
    ((normMats.getD r.val []).getD j []).getD i [] =
      expVec2 (basisAt2 r)
        (conjC 2 (invC 2 ((normGens.getD r.val []).getD j zc)) ((basisAt2 r).getD i zc)) := by
  set B := getB (classRepsData.getD r.val 0) with hB
  have hbas : basisAt2 r = B := basisAt2_eq r
  ...
  have hilen : i < (List.map (fun gi => expVec2 B (conjC 2 (invC 2 n) gi)) B).length := by
    rwa [List.length_map]
  rw [step1]
  change (List.map (fun gi => expVec2 B (conjC 2 (invC 2 n) gi)) B).getD i [] = _
  rw [List.getD_eq_getElem _ [] hilen,
    ← List.getD_eq_getElem _ (expVec2 B (conjC 2 (invC 2 n) zc)) hilen,
    List.getD_map, hbas]

/-! ## Semantic closure of the matrix orbits under genuine normalizer elements -/

/-- The `k`-th generator of `normGens r`, typed in the group `Coordinate 2`
(out-of-range index defaults to the identity). -/
def normGenAt (r k : Nat) : Coordinate 2 := (normGens.getD r []).getD k (0, 0, 0, 0, 0)
...
theorem foldl_normGen_mem (r : Fin 148) (w : List Nat) :
    w.foldl (fun acc k => acc * normGenAt r.val k) (1 : Coordinate 2) ∈
      Subgroup.normalizer ((reps r : Subgroup (Coordinate 2)) : Set (Coordinate 2)) := by
  set N := Subgroup.normalizer ((reps r : Subgroup (Coordinate 2)) : Set (Coordinate 2)) with hN
  suffices H : ∀ acc : Coordinate 2, acc ∈ N →
...
grounds the whole `N`-orbit partition (`NormOrbits.lean`) in honest normalizer
