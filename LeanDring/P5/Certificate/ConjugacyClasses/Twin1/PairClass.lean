...
/-!
# The `CharPairClass` bridge for `Coordinate 1` (the pair-class bridge)

The final step: connect the certified concrete data (the 148 certified subgroup-class
representatives `reps`, Step-2's certified character lists `charsAt`, and the normalizer-orbit
partition) to the *abstract* D-ring basis index type
`CharPairClass (Coordinate 1) ℂˣ`.

This file assembles, in stages:

* `exists_charOfVec_eq` — Step-2 **completeness as surjectivity**: every linear
  character of `reps r` is one of the certified listed characters `charOfVec r e`
  (from `charsAt_complete_all` + `charOfVec_inj`, a Nodup list of full length).
...
* `pairClass_realized_by_listed` — **surjectivity onto the pair classes**: every
  `c : CharPairClass (Coordinate 1) ℂˣ` equals `⟦(reps r, charOfVec r e)⟧` for
  some representative `r` and some *listed* certified character `e ∈ charsAt r`.
  Hence `CharPairClass (Coordinate 1) ℂˣ` is finite.
* `charForm_genAct` — **the character↔vector dictionary** (the technical heart):
  ...
  — `charForm B (M_k e) x = charForm B e (n⁻¹ x n)` for all `x ∈ reps r`, by closure
...
* `class_reduces_to_rep` / `pairClass_realized_by_orbitRep` — **sharpened
  ...
  (`P5NormOrbits.reach_from_rep`), every pair class equals the class of the
...
* `rank_dring_coordinate1_of_card` — the rank corollary, conditional on the pair
  count: `Nat.card (CharPairClass …) = 2724 ⟹ Module.rank ℤ (DRing (Coordinate 1)) = 2724`.

## Remaining for the unconditional `= 2724`

Two pieces close `Nat.card (CharPairClass (Coordinate 1) ℂˣ) = 2724` (and hence the
unconditional rank via `rank_dring_coordinate1_of_card`): (i) the routine count that
...
`charForm_genAct` identifies its action with a `MatReach` path, so the two positions
...
noncomputable def realizedChars (r : Fin 148) : List (↥(reps r) →* ℂˣ) :=
  (charsAt r).attach.map (fun p => charOfVec r p.1 (validAt r p.2))
...
theorem realizedChars_length (r : Fin 148) :
  ...
  rw [realizedChars, List.length_map, List.length_attach, charsAt_count r,
    ← charsAt_complete_all r, Nat.card_eq_fintype_card]
...
theorem exists_charOfVec_eq (r : Fin 148) (χ : ↥(reps r) →* ℂˣ) :
    ∃ e, ∃ he : e ∈ charsAt r, χ = charOfVec r e (validAt r he) := by
  classical
...
theorem conj_mem_transport (r : Fin 148) (g : Coordinate 1)
    {K : Subgroup (Coordinate 1)} (hK : conjSub g (reps r) = K)
    {k : Coordinate 1} (hk : k ∈ reps r) : g * k * g⁻¹ ∈ K := by
  rw [← hK, conjSub]
  exact Subgroup.mem_map.mpr
    ⟨k, hk, by rw [MulEquiv.coe_toMonoidHom, MulAut.conj_apply]⟩

/-- The reverse: a preimage of `K` under `g`-conjugation lies in `reps r`. -/
theorem mem_reps_of_conj (r : Fin 148) (g : Coordinate 1)
    {K : Subgroup (Coordinate 1)} (hK : conjSub g (reps r) = K)
    {k : Coordinate 1} (hk : g * k * g⁻¹ ∈ K) : k ∈ reps r := by
  rw [← hK, conjSub] at hk
...
`conjSub g (reps r) = K`. -/
noncomputable def transportChar (r : Fin 148) (g : Coordinate 1)
    {K : Subgroup (Coordinate 1)} (hK : conjSub g (reps r) = K) (l : K →* ℂˣ) :
    ↥(reps r) →* ℂˣ where
  toFun x := l ⟨g * (x : Coordinate 1) * g⁻¹, conj_mem_transport r g hK x.2⟩
  map_one' :=
...
theorem isConjPair_reps_transport (r : Fin 148) (g : Coordinate 1)
    {K : Subgroup (Coordinate 1)} (hK : conjSub g (reps r) = K) (l : K →* ℂˣ) :
...
of `Coordinate 1` is realized by a certified *listed* character on one of the 148
subgroup-class representatives: `c = ⟦(reps r, charOfVec r e)⟧` for some `r` and
some `e ∈ charsAt r`. -/
theorem pairClass_realized_by_listed (c : CharPairClass (Coordinate 1) ℂˣ) :
    ∃ (r : Fin 148) (e : List (ZMod 25)) (he : e ∈ charsAt r),
      c = (⟦⟨reps r, charOfVec r e (validAt r he)⟩⟧ : CharPairClass (Coordinate 1) ℂˣ) := by
  induction c using Quotient.inductionOn with
  | h p =>
    obtain ⟨K, l⟩ := p
    obtain ⟨r, g, hg⟩ := reps_complete K
    obtain ⟨e, he, hμe⟩ := exists_charOfVec_eq r (transportChar r g hg l)
    refine ⟨r, e, he, Quotient.sound ?_⟩
    -- `(K, l) ≈ (reps r, charOfVec r e)` : symmetric of the transport, with `μ = charOfVec`
    have hcp : IsConjPair (reps r) K (charOfVec r e (validAt r he)) l := by
...
`charForm B (M_k e) x = charForm B e (n⁻¹ x n)`. Proven by closure induction on
`x`, using Step-1 additivity for both `e` and `M_k e`, the per-entry realization
`normMats_realize`, and `dotZ_comm`. -/
theorem charForm_genAct (r : Fin 148) (k : Nat)
    (hk : k < (normGens.getD r.val []).length)
    {e : List (ZMod 25)} (he : charOK (basisAt r) e = true)
    (he' : charOK (basisAt r) (genAct r.val k e) = true)
    {x : Coordinate 1} (hx : x ∈ reps r) :
    charForm (basisAt r) (genAct r.val k e) x
      = charForm (basisAt r) e
          (((normGenAt r.val k)⁻¹ * x * (normGenAt r.val k) : Coordinate 1)) := by
  set B := basisAt r with hB
  set e' := genAct r.val k e with he'def
  obtain ⟨a, ha_def⟩ : ∃ a : Coordinate 1, a = normGenAt r.val k := ⟨_, rfl⟩
  rw [← ha_def]
  -- normalizer facts
  have hnmem : a ∈
      Subgroup.normalizer ((reps r : Subgroup (Coordinate 1)) : Set (Coordinate 1)) := by
    rw [ha_def]; exact normGen_getD_mem r k
  have hnormiff : ∀ y : Coordinate 1, y ∈ reps r ↔ a⁻¹ * y * a ∈ reps r := by
    intro y
    have h := (Subgroup.mem_normalizer_iff''.mp hnmem) y
    simpa only [SetLike.mem_coe] using h
  have hconjmem : ∀ {y : Coordinate 1}, y ∈ reps r → a⁻¹ * y * a ∈ reps r :=
    fun hy => (hnormiff _).mp hy
  -- charForm additivity helpers on `reps r`
  have haddE : ∀ (v : List (ZMod 25)), charOK B v = true → ∀ {c d : Coordinate 1},
      c ∈ reps r → d ∈ reps r →
      charForm B v ((c * d : Coordinate 1)) = charForm B v c + charForm B v d := by
    intro v hv c d hc hd
    exact charExtendsAt_basisAt r v hv c d (siftB_of_mem r ⟨c, hc⟩) (siftB_of_mem r ⟨d, hd⟩)
  have hone : ∀ (v : List (ZMod 25)), charOK B v = true →
      charForm B v (1 : Coordinate 1) = 0 := by
    intro v hv
    have h1 := charOfVec_apply r v hv (1 : ↥(reps r))
    rw [map_one, Subgroup.coe_one] at h1
    exact zexp_inj (h1.symm.trans zexp_zero.symm)
  have hnegE : ∀ (v : List (ZMod 25)), charOK B v = true → ∀ {c : Coordinate 1},
      c ∈ reps r → charForm B v ((c⁻¹ : Coordinate 1)) = - charForm B v c := by
    intro v hv c hc
    have h := haddE v hv (inv_mem hc) hc
    rw [inv_mul_cancel, hone v hv] at h
    exact eq_neg_of_add_eq_zero_left h.symm
  -- the conjugation-by-`a` group identities
  have hsplit : ∀ x y : Coordinate 1, a⁻¹ * (x * y) * a = (a⁻¹ * x * a) * (a⁻¹ * y * a) := by
    intro x y; group
  have hinvsplit : ∀ x : Coordinate 1, a⁻¹ * x⁻¹ * a = (a⁻¹ * x * a)⁻¹ := by
    intro x; group
  have honesplit : a⁻¹ * (1 : Coordinate 1) * a = 1 := by group
  -- generator-evaluation of a valid vector
  have hgenEval : ∀ (v : List (ZMod 25)), charOK B v = true → ∀ i (hi : i < B.length),
      charForm B v (B.getD i zc) = v.getD i 0 := by
    intro v hv i hi
    have hmem : ((B.getD i zc : Coordinate 1)) ∈ reps r := by
      rw [reps_eq_ofList r, ← hB, List.getD_eq_getElem B zc hi]
      exact Subgroup.subset_closure (List.getElem_mem hi)
    have hg := charOfVec_gen r v hv i hi hmem
    rw [charOfVec_apply] at hg
    exact zexp_inj hg
  ...
  have hMlen : ((normMats.getD r.val []).getD k []).length = B.length := by
    ...
    rw [normRealize_at r, List.getD_eq_getElem _ [] hklt, List.getElem_map, conjMatrixL2,
      List.length_map, ← basisAt_eq r, ← hB]
  ...
  have hbridge : ∀ g : Coordinate 1,
      (conjC 1 (invC 1 ((normGens.getD r.val []).getD k zc)) g : Coordinate 1)
        = a⁻¹ * g * a := by
    intro g
    rw [conjC_eq hq1]
    have h1 : (invC 1 ((normGens.getD r.val []).getD k zc) : Coordinate 1) = a⁻¹ := by
      rw [ha_def]; rfl
    rw [h1, inv_inv]
  -- closure induction
  have toReps : ∀ {c : Coordinate 1},
      c ∈ Subgroup.closure {y : Coordinate 1 | y ∈ B} → c ∈ reps r := by
    intro c hc; rw [reps_eq_ofList r, ← hB]; exact hc
  have hxc : x ∈ Subgroup.closure {y : Coordinate 1 | y ∈ B} := by
    rw [reps_eq_ofList r, ← hB] at hx; exact hx
  refine Subgroup.closure_induction
    (p := fun (z : Coordinate 1) _ => charForm B e' z = charForm B e ((a⁻¹ * z * a : Coordinate 1)))
...
listed character, then `(reps r, charOfVec (genAct r k e))` and
`(reps r, charOfVec e)` are `G`-conjugate pairs (via the normalizer generator
`normGens r [k]`), so they define the *same* `CharPairClass`. -/
theorem class_genAct_eq (r : Fin 148) (k : Nat)
    (hk : k < (normGens.getD r.val []).length)
    {e : List (ZMod 25)} (he : e ∈ charsAt r) (he' : genAct r.val k e ∈ charsAt r) :
    (⟦⟨reps r, charOfVec r (genAct r.val k e) (validAt r he')⟩⟧ :
        CharPairClass (Coordinate 1) ℂˣ)
      = ⟦⟨reps r, charOfVec r e (validAt r he)⟩⟧ := by
  ...
  · -- the character transports along the conjugation
    have hcf := charForm_genAct r k hk (validAt r he) (validAt r he') (x := k0) hk0
    rw [← ha_def] at hcf
    rw [charOfVec_apply, charOfVec_apply]
    change zexp (charForm (basisAt r) e (a⁻¹ * k0 * (a⁻¹)⁻¹))
      = zexp (charForm (basisAt r) (genAct r.val k e) k0)
    rw [inv_inv, hcf]

/-- A position `p < |charsAt r|` reads a listed character. -/
theorem listedAt (r : Fin 148) {p : Nat} (hp : p < (repChars.getD r.val []).length) :
    (repChars.getD r.val []).getD p [] ∈ charsAt r := by
  change (repChars.getD r.val []).getD p [] ∈ repChars.getD r.val []
...
theorem class_charOfVec_congr (r : Fin 148) {v w : List (ZMod 25)}
    (hv : v ∈ charsAt r) (hw : w ∈ charsAt r) (h : v = w) :
    (⟦⟨reps r, charOfVec r v (validAt r hv)⟩⟧ : CharPairClass (Coordinate 1) ℂˣ)
      = ⟦⟨reps r, charOfVec r w (validAt r hw)⟩⟧ := by
  subst h; rfl
...
theorem class_reduces_to_rep (r : Fin 148) : ∀ (dep p : Nat)
    (hp : p < (repChars.getD r.val []).length)
    (_hdep : (normTreeDepth.getD r.val []).getD p 0 = dep),
    ∃ q, ∃ hq : q < (repChars.getD r.val []).length,
      (normIsRep.getD r.val []).getD q false = true ∧
      (⟦⟨reps r, charOfVec r ((repChars.getD r.val []).getD p []) (validAt r (listedAt r hp))⟩⟧ :
          CharPairClass (Coordinate 1) ℂˣ)
  ...
  | _ dep IH =>
    ...
    · have hrf : (normIsRep.getD r.val []).getD p false = false := by
      ...
      have hgenActeq : genAct r.val k ((repChars.getD r.val []).getD j []) =
          (repChars.getD r.val []).getD p [] := by rw [genAct, haf.1, hmove]
      have hnglen : k < (normGens.getD r.val []).length := by
        ...
        rw [← hlen]; exact hklt
      have he' : genAct r.val k ((repChars.getD r.val []).getD j []) ∈ charsAt r := by
        rw [hgenActeq]; exact listedAt r hp
      have hstep := (class_charOfVec_congr r (listedAt r hp) he' hgenActeq.symm).trans
...
`G`-conjugacy class of character pairs of `Coordinate 1` equals the class of the
...
`|CharPairClass| ≤ 2724`. -/
theorem pairClass_realized_by_orbitRep (c : CharPairClass (Coordinate 1) ℂˣ) :
    ∃ (r : Fin 148) (q : Nat) (hq : q < (repChars.getD r.val []).length),
      (normIsRep.getD r.val []).getD q false = true ∧
      c = ⟦⟨reps r, charOfVec r ((repChars.getD r.val []).getD q [])
        (validAt r (listedAt r hq))⟩⟧ := by
  obtain ⟨r, e, he, hce⟩ := pairClass_realized_by_listed c
  have hmem : e ∈ repChars.getD r.val [] := he
  obtain ⟨p, hp, hpe⟩ := List.getElem_of_mem hmem
  have hpe' : (repChars.getD r.val []).getD p [] = e := by
    rw [List.getD_eq_getElem _ [] hp, hpe]
  obtain ⟨q, hq, hqrep, hred⟩ := class_reduces_to_rep r _ p hp rfl
  refine ⟨r, q, hq, hqrep, ?_⟩
  rw [hce]
  exact (class_charOfVec_congr r he (listedAt r hp) hpe'.symm).trans hred

/-! ## Finiteness of the pair-class index and the rank corollary -/

/-- The pair classes of `Coordinate 1` form a finite type (surjective image of the
finite set of `(representative, listed character)` pairs). -/
noncomputable instance : Finite (CharPairClass (Coordinate 1) ℂˣ) := by
  classical
  -- surjection from `Σ r, {e // e ∈ charsAt r}` (a finite type) onto the classes
  apply Finite.of_surjective
    (f := fun p : (Σ r : Fin 148, {e : List (ZMod 25) // e ∈ charsAt r}) =>
      (⟦⟨reps p.1, charOfVec p.1 p.2.1 (validAt p.1 p.2.2)⟩⟧ :
        CharPairClass (Coordinate 1) ℂˣ))
  intro c
...
index has cardinality `2724`, then `rank_ℤ D(Coordinate 1) = 2724`. Composes
`DRing.basisOfCharPairClass` with `rank_eq_card_basis`. -/
theorem rank_dring_coordinate1_of_card
    (h : Nat.card (CharPairClass (Coordinate 1) ℂˣ) = 2724) :
    Module.rank ℤ (DRing (Coordinate 1)) = 2724 := by
  classical
  haveI : Fintype (CharPairClass (Coordinate 1) ℂˣ) := Fintype.ofFinite _
  have hcard : Fintype.card (CharPairClass (Coordinate 1) ℂˣ) = 2724 := by
    rw [← Nat.card_eq_fintype_card]; exact h
  rw [rank_eq_card_basis (DRing.basisOfCharPairClass (G := Coordinate 1)), hcard]
  norm_cast
