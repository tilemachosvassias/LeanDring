...
/-!
# `SpeciesColsComplete` for `Coordinate 2`: the concrete `q = 2` coverage
...
* **Step 1** (`species_col_conj_to_reps`, `Twin2/SpeciesColumn.lean`) — conjugate any column
...
The capstone `speciesColsComplete_coordinate2` packages the orbit-representative
...
theorem transElt_mem (r : Fin 148) {p : Nat}
  ...
  have hs : siftB 2 (getB (classRepsData.getD r.val 0))
...
theorem hpSub_reduceAux (r : Fin 148) (a b : Coordinate 2)
    (hab : a⁻¹ * b ∈ hpSub r.val) :
    reduceAux 2 (hpData.getD r.val []) a = reduceAux 2 (hpData.getD r.val []) b := by
  have hthis := reduceAux_mul_mem hq2 (hpData.getD r.val []) (hpSub_wf r) a (a⁻¹ * b) hab
...
theorem reduceAux_trans_inj (r : Fin 148) {p p' : Nat}
    (hp : p < (transData.getD r.val []).length)
    (hp' : p' < (transData.getD r.val []).length)
    (h : reduceAux 2 (hpData.getD r.val []) ((transData.getD r.val []).getD p zc)
       = reduceAux 2 (hpData.getD r.val []) ((transData.getD r.val []).getD p' zc)) :
    p = p' := by
  set red := (transData.getD r.val []).map (reduceAux 2 (hpData.getD r.val [])) with hred
  ...
  have hpl' : p' < red.length := by rw [hred, List.length_map]; exact hp'
  have hgp : red[p] =
      reduceAux 2 (hpData.getD r.val []) ((transData.getD r.val []).getD p zc) := by
...
open LeanDring.P5Presentation (Coordinate zc reduceAux sift_mem_iff hq2)
open scoped commutatorElement
...
theorem species_eq_of_hpSub (r : Fin 148) (a b : ↥(reps r))
    (hab : (a : Coordinate 2)⁻¹ * (b : Coordinate 2) ∈ hpSub r.val) :
    species (reps r) a = species (reps r) b := by
  have hsubcoe : (reps r).subtype (a⁻¹ * b) = (a : Coordinate 2)⁻¹ * (b : Coordinate 2) := by
...
theorem exists_trans_pos (r : Fin 148) (h : ↥(reps r)) :
  ...
  have hcard : Fintype.card (Fin L) = Fintype.card (Abelianization ↥(reps r)) := by
    rw [Fintype.card_fin, ← Nat.card_eq_fintype_card, card_abelianization_reps_all2, hL,
      transCard]
  have hinj : Function.Injective f := by
    ...
    have hmap : ((⟨(transData.getD r.val []).getD p.val zc, transElt_mem r p.isLt⟩ :
            ↥(reps r)) : Coordinate 2)⁻¹
        * ((⟨(transData.getD r.val []).getD p'.val zc, transElt_mem r p'.isLt⟩ :
            ↥(reps r)) : Coordinate 2) ∈ hpSub r.val := by
...
theorem map_conj_normGen (r : Fin 148) (k : Nat) :
  ...
  have hnormiff : ∀ y : Coordinate 2, y ∈ reps r ↔ n⁻¹ * y * n ∈ reps r := by
    intro y
...
theorem species_cosetStep (r : Fin 148) (k p : Nat)
  ...
  set n := normGenAt r.val k with hn
  -- Genuine `Coordinate 2` handles for the two transversal columns, so every group
  -- operation below is elaborated at `Coordinate 2` (via the subtype coercion `↑a`).
  ...
  have hnormiff : ∀ y : Coordinate 2, y ∈ reps r ↔ n⁻¹ * y * n ∈ reps r := by
    ...
    have h := (Subgroup.mem_normalizer_iff''.mp hnmem) y
    simpa only [SetLike.mem_coe] using h
  have wmem : n⁻¹ * (a : Coordinate 2) * n ∈ reps r := (hnormiff _).mp a.2
  -- the certified `H'`-membership, as `T[p']⁻¹ · (n⁻¹ · T[p] · n) ∈ H'`
  have hmemHp : (a' : Coordinate 2)⁻¹ * (n⁻¹ * (a : Coordinate 2) * n) ∈ hpSub r.val :=
    (sift_mem_iff hq2 _ (hpSub_wf r) _).mp hsift
  -- conjugation move
  have hg : (reps r).map (MulAut.conj n).toMonoidHom = reps r := map_conj_normGen r k
  have hconj : species (reps r) a
      = species (reps r) (⟨n⁻¹ * (a : Coordinate 2) * n, wmem⟩) :=
    species_of_conjSub (reps r) ⟨n⁻¹ * (a : Coordinate 2) * n, wmem⟩ n hg a (by group)
  -- residual `H'`-shift
  have hshift : species (reps r) a'
      = species (reps r) (⟨n⁻¹ * (a : Coordinate 2) * n, wmem⟩) :=
    species_eq_of_hpSub r a' ⟨n⁻¹ * (a : Coordinate 2) * n, wmem⟩ hmemHp
  rw [hshift, ← hconj]
...
/-- The `2724` orbit-representative species columns of `Coordinate 2`, indexed by a
...
def orbitRepColumns :
    (Σ r : Fin 148, {p : Nat //
      ...
      → SpeciesColumn (Coordinate 2) :=
  fun x =>
    ⟨reps x.1, ⟨(transData.getD x.1.val []).getD x.2.val zc, transElt_mem x.1 x.2.2.1⟩⟩

/-- **Completeness capstone.** The orbit-representative columns form a complete family of
species columns for `Coordinate 2`: every species column `(H, h)` has the same
...
/-- **Completeness glue (column half).** Whenever a source family `colsG` and a row
bijection `σ` reproduce the `2724` orbit-representative columns of
`Coordinate 2`, the column-completeness proved here
(`speciesColsComplete_coordinate2`) upgrades that match to an abstract `D`-ring
isomorphism `DRing G ≃+* DRing (Coordinate 2)`.

The row-side hypotheses `σ` and `hmatch` are discharged, not assumed:
`LeanDring/P5/Certificate/RingIsomorphism/Twin1/Assembly.lean` supplies them as
`P5Presentation.sigmaEquiv` and
`P5Presentation.hmatch`, and this lemma — applied with `G := Coordinate 1` and
`colsG := colsG1` — *is* the proof of the unconditional
`P5Presentation.dring_equiv_coordinate12`. -/
theorem dringEquiv_of_rowMatch_coordinate2 {G : Type} [Group G] [Fintype G]
  ...
  nonempty_dringEquiv_of_reduced colsG orbitRepColumns
    speciesColsComplete_coordinate2 σ hmatch
