...
/-!
# Column-side enumeration for `q = 1`: the `H' = [H,H]` semantic squeeze

Turns the eight boolean certificates of `Twin1/SpeciesCosets.lean` into the group-theoretic
identity they encode: the emitted per-rep candidate `hpData r` genuinely presents
the derived subgroup `[H,H]` of `H = reps r`.

The squeeze is by **cardinality**, using the character-side bridge
...
theorem hpWf_at (r : Fin 148) : wfB' 1 (hpData.getD r.val []) = true := by
...
def hpSub (r : Nat) : Subgroup (Coordinate 1) := ofList 1 hq1 (hpData.getD r [])

/-- `H'` is a well-formed echelon pcgs. -/
theorem hpSub_wf (r : Fin 148) : WF 1 hq1 (hpData.getD r.val []) :=
  wf_of_wfB' hq1 _ (hpWf_at r)

/-- `|H'| = 5 ^ (length of its pcgs)`. -/
theorem card_hpSub (r : Fin 148) :
    Nat.card (hpSub r.val) = 5 ^ (hpData.getD r.val []).length :=
  card_ofList hq1 _ (hpSub_wf r)
...
theorem hpSub_le_reps (r : Fin 148) : hpSub r.val ≤ reps r := by
  ...
  have hs : siftB 1 (getB (classRepsData.getD r.val 0)) g = true := h g hg
...
theorem reps_le_normalizer_hpSub (r : Fin 148) :
  ...
  change b ∈ Subgroup.normalizer (Subgroup.closure {x : Coordinate 1 | x ∈ hpData.getD r.val []})
  apply normalizes_closure hq1 b {x : Coordinate 1 | x ∈ hpData.getD r.val []}
  · intro g hg
    have hs : siftB 1 (hpData.getD r.val []) (conjByC b g) = true := (h g hg b hb).1
    have hmem : conjByC b g ∈ ofList 1 hq1 (hpData.getD r.val []) :=
      (sift_mem_iff hq1 _ (hpSub_wf r) _).mp hs
    rwa [show conjByC b g = conjC 1 b g from rfl, conjC_eq hq1 b g] at hmem
  · intro g hg
    have hs : siftB 1 (hpData.getD r.val []) (conjByC (invC 1 b) g) = true := (h g hg b hb).2
    have hmem : conjByC (invC 1 b) g ∈ ofList 1 hq1 (hpData.getD r.val []) :=
      (sift_mem_iff hq1 _ (hpSub_wf r) _).mp hs
    rw [show conjByC (invC 1 b) g = conjC 1 (invC 1 b) g from rfl, conjC_eq hq1 (invC 1 b) g,
      show invC 1 b = b⁻¹ from rfl, inv_inv] at hmem
    exact hmem

/-- Generator-commutators of `H` land in `H'`. -/
theorem gen_comm_mem_hpSub (r : Fin 148) :
    ∀ a ∈ {x : Coordinate 1 | x ∈ getB (classRepsData.getD r.val 0)},
      ∀ b ∈ {x : Coordinate 1 | x ∈ getB (classRepsData.getD r.val 0)},
      ⁅a, b⁆ ∈ hpSub r.val := by
  intro a ha b hb
  simp only [Set.mem_setOf_eq] at ha hb
  have h := commIn_at r
  simp only [commInCheck, spIdx, List.all_eq_true] at h
  have hs : siftB 1 (hpData.getD r.val []) (commutatorC a b) = true := h a ha b hb
  have hmem : commutatorC a b ∈ ofList 1 hq1 (hpData.getD r.val []) :=
    (sift_mem_iff hq1 _ (hpSub_wf r) _).mp hs
...
theorem commutator_reps_le_hpSub (r : Fin 148) :
  ...
  refine commutator_gens_mem
    (S := {x : Coordinate 1 | x ∈ getB (classRepsData.getD r.val 0)})
...
theorem hpSub_eq_derived (r : Fin 148) :
  ...
  have hab : Nat.card (Abelianization (reps r)) = charCounts.getD r.val 0 :=
    card_abelianization_reps_all r
