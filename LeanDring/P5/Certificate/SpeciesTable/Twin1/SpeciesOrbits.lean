...
-/
import LeanDring.P5.Data.SpeciesOrbitChunk.C001
import LeanDring.P5.Data.SpeciesOrbitChunk.C003
import LeanDring.P5.Data.SpeciesOrbitChunk.C005
import LeanDring.P5.Data.SpeciesOrbitChunk.C007
import LeanDring.P5.Data.SpeciesOrbitChunk.C009
import LeanDring.P5.Data.SpeciesOrbitChunk.C011
import LeanDring.P5.Data.SpeciesOrbitChunk.C013
import LeanDring.P5.Data.SpeciesOrbitChunk.C015
import LeanDring.P5.Data.SpeciesOrbitChunk.C017
import LeanDring.P5.Data.SpeciesOrbitChunk.C019
import LeanDring.P5.Data.SpeciesOrbitChunk.C021
import LeanDring.P5.Data.SpeciesOrbitChunk.C023
import LeanDring.P5.Certificate.ConjugacyClasses.Twin1.NormLink

/-!
# Column-side coset-orbit partition (`q = 1`)

Assembly of the twelve chunked kernel checks (`cosetOrbChunk0`–`11`, in
`LeanDring/P5/Data/SpeciesOrbitChunk/`) into
...
in `Twin1/SpeciesLink.lean`, `hpSub_eq_derived`) and transversal `T = transData r` (certified
...
* `speciesOrbitTotal` — `Σ_r cosetOrbitCountR r = 2724`, the pair-class transversal
  size (`= rank D(G)` for `q = 1`), **matching the character side** (`pairClassTotal`);
* `speciesOrbitCount_distribution` — the per-rep orbit counts realise the same
  fingerprint `{1:1, 5:12, 9:61, 13:5, 25:56, 29:1, 45:11, 125:1}` as the
  character side;
...
mirrors the character side's `MatReach` layer on positions.
...
`LeanDring/P5/Certificate/ConjugacyClasses/Twin1/NormLink.lean`) over the
connecting word. The per-step realization (table entry = conjugate coset) is
`cosetActOK` in the data file.
-/
...
`rank D(G)` (`q = 1`) obtained on the character side (`pairClassTotal`). -/
...
the column-side counterpart of the character side's `MatReach`. Every coset
reaches its orbit representative along the spanning tree, and its label is stable
under every generator's action. -/
...
`normGens_generate` (`ConjugacyClasses/Twin1/NormLink.lean`) certifies that
