# bc_separation — Theorem BC-n for all `n ≥ 3`: the separation of `B_n(q)` from
# `C_n(q)` is by a single `ind*` VALUE

Scripts `orthogonal_witness.g` (GAP 4.16),
`symplectic_exclusion.py`; outputs `orthogonal_witness_out.txt`,
`symplectic_exclusion_out.txt`.

---

## 0. Summary

The separation is **not** through parabolic indices.

A second index invariant `ι₂` *read as a min-of-parabolic-indices* ties:
`B₃` and `C₃` are polar spaces of rank 3 with the *same* parameters
`(q, 1)`, so **every** parabolic index of `Ω₇(q)` equals the corresponding
index of `Sp₆(q)`.  The comparison `[Sp₆:P₂]` vs `[Ω₇:P₂]` is a tie,
identically in `q`, and so is `[·:P₃]`.  The separation comes instead from a
**non-parabolic** `ind*` value that exists on the orthogonal side and cannot
exist on the symplectic side:
...
This supersedes the earlier `mult₁`/`Δ(q)` route for `B/C`.  The
`B_n ≅_{order} C_n` simple–simple pair was the last obstruction left in the
symplectic/orthogonal towers; with it closed, `Sp_{2n}(q)` and `Ω_{2n+1}(q)`
are D-rigid among all finite groups for every `n ≥ 3` and every `q`
(`q` even is vacuous: `B_n(2^f) ≅ C_n(2^f)`).
...
By Müller, Diss. Jena 2008, Satz 2.3.3 the pair `(H, H')` has conductor
`|N_G(H)|/|H'|`, so
...
convention is not consumed beyond its standing use elsewhere.
...
**GAP verification** (`orthogonal_witness.g` PART A, `Ω(7,q)` as a matrix
...
**The maximal-subgroup step, checked exactly** (`symplectic_exclusion.py`
...
1. **Theorem BC-n is `n`-uniform, unconditional, and cheap.**  The former
   tie set of the `mult₁` route — `q = 11`, `49`, `121`, `169` — is settled
...
3. **No `Δ`-tie-breaker is needed.**  There is no `Δ` to restate over
   `(p, r)`, no tie set to characterise, and no tie-breaker to deploy; no
   census is built on the `C₅`-subfield bookkeeping or the `Sym⁵ SL₂`
   column any more.
...
5. **Combined with the symplectic/orthogonal twin theorems** (Sp-T,
   O-odd-T): `Sp_{2n}(q)` and `Ω_{2n+1}(q)` are **D-rigid among all finite
   groups** for `n ≥ 3`, all `q`.  The classical-group programme has no
   `B/C` residue left.

---

## 6. Status of each step

| statement | status |
|---|---|
| `I(G)` is a `D`-invariant | proven (Müller, Diss. Jena 2008, Satz 2.3.3 + kernel-checked conductor invariance + order transfer) |
| `v_ε ∈ I(Ω_{2n+1}(q))`, `ε` determined by `disc(V)` | proven by hand (§2); GAP check at `q = 3,5,7,9`, `n = 3` agrees |
| the two anisotropic orbit sizes are `q^n(q^n∓1)/2` | proven (standard); GAP check agrees |
| `P₁` is the only maximal of `Sp_{2n}(q)` of index `< q^{2n}` | proven modulo literature — Aschbacher/BHR classification (cited), Liebeck's `S`-bound for `n ≥ 4`, BHR 8.28/8.29 orders for `n = 3`; exact numerical check `n ≤ 8`, `q < 200` |
