...
# -*- coding: utf-8 -*-
r"""
rank1_gaps.py -- closing the residual rank-1 gaps (G2'), (G3), (G4)
left open by the development record, for the targets

      S = U_3(q),   Sz(q) = ^2B_2(q),   R(q) = ^2G_2(q).

Standing hypotheses (all PROVEN upstream, see the .md):
   T Fitting-free, |T| = |S|, D(T) = D(S);  Steps 2-3 of rank1_twins (which now
   rest on Lemma C, PROVEN in lemma_c_bound) give  Soc T = S_1  SIMPLE  and
...
PARTS
  ...
  6  (G3) the residual window vs the rank1_twins --sub scan range.
...
import lemma_c_bound as C52
from lemma_c_bound import FAM, gcd, order_of, _prod
from rank1_twins import (U3_order, Sz_order, Ree_order,
...
def part2(verbose=True):
    ...
    print(BAR)
    print("""
  (T1)  |S_1| >= q_1^{D1}/(12 d1)          [lemma_c_bound]
  (T2)  iota(S_1) <= 4 q_1^{mu1}           [lemma_c_bound]
...
def part6():
    ...
    print(BAR)
    print("""
  rank1_twins (G3) complained that the direct scan of (E) used q_1 <= 10^5 and
  Lie rank <= 6.  After PARTS 3-5 that scan is no longer load-bearing:
    ...
    U_3 : cross characteristic dies by rank1_twins Sec 5.2 for q >= 179;
...
""")
    ...
    print()
    print("  Alternating / sporadic socles: rank1_twins Sec 5.4 is COMPLETE in q")
