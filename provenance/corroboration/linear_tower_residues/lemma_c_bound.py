...
# -*- coding: utf-8 -*-
"""
lemma_c_bound.py -- machine half of the PROOF of Lemma C, and of the
closure of the residual branches (G2) of l2_twins.
...
The hand proof (the development record) reduces Lemma C for the groups of Lie
type to
...
Run:   python3 lemma_c_bound.py > lemma_c_bound_out.txt
"""
...
fam(key="E7", name=lambda n, q: "E7(%d)" % q, nmin=1, nmax=1,
    ...
    terms=lambda n, q: [q ** 18 - 1, q ** 14 - 1, q ** 12 - 1, q ** 10 - 1,
                        q ** 8 - 1, q ** 6 - 1, q ** 2 - 1],
    d=lambda n, q: gcd(2, q - 1), g=1,
    # ERRATUM (the development record): was (q^14-1)(q^9+1)(q^5-1)/((q^2-1)(q-1)) with
...
fam(key="E8", name=lambda n, q: "E8(%d)" % q, nmin=1, nmax=1,
    ...
    d=lambda n, q: 1, g=1,
    # ERRATUM (the development record): was divided by an extra (q^2-1)(q^4-1) with
...
IOTA_ALT_EXACT = {5: 5, 6: 10}      # GAP; C(m,3) is the bound m >= 7
...
def part4():
    print()
    print("=" * 72)
    print("PART 4 -- sporadic groups (exact iota, sporadic_twins)")
    print("=" * 72)
    from sporadic_twins import SPORADIC, TITS, IOTA_UB
    bad, worst = 0, (0.0, "")
...
def part5():
    ...
    print("       2^(v2(m!)-1) <= 2*|Out(A_m)|*iota(A_m) fails for all m > %d"
          % mmax)
    print("       ==> m <= %d; that range is exhausted by the l2_twins scan"
          % mmax)
    ...
    print("       finite list, scanned in l2_twins with 0 hits.")
    return 0
