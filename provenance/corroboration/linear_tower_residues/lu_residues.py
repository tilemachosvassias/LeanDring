...
# -*- coding: utf-8 -*-
r"""
lu_residues.py -- closing the three named residues of lu_twins
(Theorem LU-T for the LINEAR and UNITARY towers, S = L_d(q), U_d(q), d >= 4)

The residues (lu_twins section 8, "Honest gaps"):
...
Four new tools, all target-agnostic:
  ...
  * Lemma DEG (the degree partition).  The p-part comparison is an EQUALITY
    (Lemma EQ-EX, the development record), so with a := deg_q|S| = d^2-1, N := d(d-1)/2,
...
from lemma_c_bound import FAM, order_of, out_of
from lu_twins import (KFAM, bdeg, Kmax, order_LU, iota_LU, out_LU,
...
def partB():
  ...
  Lemma Z of lu_twins is exactly the case kappa = k_i, m = 1, M_t >= 1.
...
""")
  ...
  q <= 2*10^6 for the equal-characteristic sweep (lu_twins PART C.2),
...
def partE_H3(dlist=range(11, 21)):
    ...
    print(BAR)
    print("""
  The four finite tests of lu_twins (C.2 equal-characteristic almost-simple
...
""")
    ...
    from lu_twins import partC_window
...
def partF_tail(dmax=200):
  ...
  > (lu_twins's Lemma X had d! in place of 2^{d-1}.)
  ...
  >     lu_twins section 3 closes at q^{(d-1)/(d+2)} <= 8 d_1 f_1 with
